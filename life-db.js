	...
	*/
	checkAndRestoreData = (function() {
		...
		if(!isNodeEnvironment && isSessionStorageAvaileble) {
			if(typeof sessionStorage[databaseName] === "undefined" || sessionStorage[databaseName].trim()==="") {
				if(typeof window[databaseName] === "undefined") {
					// empty window so Restore process ignored	
				} else {
					if(typeof window[databaseName] === "object") {
						globalDataStorage = window[databaseName];
					} else {
				...
				if(sessionStorage[databaseName].trim() !== "") {console.log("8");
					globalDataStorage = JSON.parse(sessionStorage[databaseName]);
				} else {
